...
export function showErrorModal(message: string, title: string = 'An error occurred'): void {
    ...
    const errorModal = Modal.getInstance(modalElement) || new Modal(modalElement);
...
export function showSuccessModal({ message, title, afterCloseModal }: ShowSuccessModalParams): void {
...
export function createQuestion(options: QuestionOptions): void {
    ...
    if (confirmModalText && confirmModalTitle && confirmButton && cancelButton) {
        ...
        const confirmFn = function () {
            ...
            const confirmModal = Modal.getInstance(document.getElementById('confirmModal')!);
            if (confirmModal) {
                confirmModal.hide();
            }
        };
        ...
        if (element) {
            const confirmModal = new Modal(element);
            confirmModal.show();
